...
Usage:
    ...
    # Terminal 1 (creator):
    python3 cli.py create_session --policy ScheduleOverlap --participants 2
    python3 cli.py get_status --token <TOK>
    python3 cli.py submit_input --token <TOK> --input '{"available_slots":["2026-07-01T10:00:00Z"]}'
    python3 cli.py get_result --token <TOK>

    # Terminal 2 (joiner):
    python3 cli.py join_session --invitation_token <ITOK>
    python3 cli.py accept_policy --token <TOK>
    python3 cli.py submit_input --token <TOK> --input '{"available_slots":["2026-07-01T10:00:00Z","2026-07-01T14:00:00Z"]}'
    python3 cli.py get_result --token <TOK>
"""
...
def _build_policy(args) -> VerifierPolicy:
    """Construct a VerifierPolicy from CLI args."""
    name = args.verifier
    if name == "noop":
        return NoopPolicy()
    elif name == "ita":
        digests = frozenset()
        digest_arg = getattr(
            args, "expected_digest", None
        )
        if digest_arg:
            digests = frozenset(
                d.strip()
                for d in digest_arg.split(",")
                if d.strip()
            )
        kwargs = dict(
            expected_image_digests=digests,
            allow_debug=getattr(
                args, "allow_debug_tee", False
            ),
            expected_project_id=getattr(
                args, "expected_project_id", ""
            ),
            expected_service_account=getattr(
                args, "expected_service_account", ""
            ),
        )
        min_ver = getattr(
            args, "min_cs_version", None
        )
        if min_ver is not None:
            kwargs["min_cs_version"] = min_ver
...
def _make_stub(args):
    ...
    stub = (
        ...
        .AgentBrokerServiceStub(
            channel_wrapper.grpc_channel
        )
    )
...
def _add_connection_args(parser):
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Optional GCP project ID to enforce.",
    )
    parser.add_argument(
        ...
        default="",
        help=(
            "Optional GCP service account to enforce."
        ),
    )
    parser.add_argument(
        "--min_cs_version",
        ...
        default=None,
        help=(
            "Minimum Confidential Space image "
            "version (e.g. 260500). "
            "Default: 260500."
        ),
    )
    parser.add_argument(
        ...
        "--quiet",
        action="store_true",
        help="Suppress attestation details",
    )
...
def cmd_echo(args):
    ...
    try:
        request = session_manager_pb2.EchoRequest(
            message=args.message
        )
        print(
            f"Sending Echo request: '{args.message}'"
        )
...
def cmd_create_session(args):
    ...
    try:
        ...
        if args.input_schema:
            policy.input_schema_json = (
                args.input_schema
            )
        if args.output_schema:
            policy.output_schema_json = (
                args.output_schema
            )
        ...
        if creator_token:
            metadata.append(
                (
                    'x-ztab-creator-token',
                    creator_token,
                )
            )

        request = (
            session_manager_pb2.CreateSessionRequest(
                policy=policy,
...
def cmd_join_session(args):
    ...
    try:
        request = (
            session_manager_pb2.JoinSessionRequest(
                invitation_token=(
                    args.invitation_token
                ),
                role=args.role or "",
        ...
        print(
            f"  policy_class: "
            f"{response.policy.policy_class}"
        )
        print(
            f"  expected:     "
            f"{response.policy.expected_participants}"
            f" participants"
        )
        print(
            f"  token:        "
...
def cmd_accept_policy(args):
    ...
    try:
        request = (
            session_manager_pb2.AcceptPolicyRequest(
        ...
        state_name = (
            session_manager_pb2.SessionState.Name(
                response.state
            )
        )
        ...
        if (
            ...
            print(
                f"You can now submit your input."
            )
        else:
            print(
...
def cmd_submit_input(args):
    ...
    try:
        ...
        request = (
            session_manager_pb2.SubmitInputRequest(
                participant_token=args.token,
        ...
        print(
            f"  remaining: "
            f"{response.remaining_inputs} "
            f"inputs needed"
        )
        if response.remaining_inputs == 0:
            print(
                f"\nAll inputs received. "
                f"LLM processing has started."
            )
            print(
                f"Call get-result to retrieve "
                f"the output."
            )
        else:
            print(
...
def cmd_get_result(args):
    ...
    try:
        request = (
            session_manager_pb2.GetResultRequest(
                participant_token=args.token,
            )
        )
        ...
        if (
            ...
            print(
                ...
                f"Session is still {state_name}."
            )
            print(
                f"Try again after all inputs "
                f"are submitted."
            )
    finally:
        channel.close()
...
def cmd_get_status(args):
    ...
    try:
        ...
        print(
            f"  joined:   "
            f"{response.participants_joined}"
        )
        print(
            f"  accepted: "
            f"{response.participants_accepted}"
        )
        print(
            f"  inputs:   "
            f"{response.inputs_received}"
        )
    finally:
        channel.close()
...
def build_parser():
    parser = argparse.ArgumentParser(
        description=(
            "ZTAB CLI — Agent Broker client for "
            "Echo and Session RPCs."
        ),
    )
    ...
    echo_parser = subparsers.add_parser(
        "echo",
        help="Send an Echo RPC (health check)",
    )
    ...
    create_parser = subparsers.add_parser(
        "create_session",
        help="Create a new multi-agent session",
    )
    _add_connection_args(create_parser)
    create_parser.add_argument(
        "--policy",
        default="ScheduleOverlap",
    ...
    create_parser.add_argument(
        ...
        type=int,
        default=2,
        help="Expected participant count",
    )
    create_parser.add_argument(
        "--timeout",
        type=int,
    ...
    create_parser.add_argument(
        ...
        default="",
        help=(
            "Creator token for admission control "
            "(optional)"
        ),
    )
    create_parser.add_argument(
    ...
    join_parser.add_argument(
        ...
        required=True,
        help=(
            "Invitation token from session creator"
        ),
    )
    join_parser.add_argument(
        "--client_nonce",
    ...
    accept_parser = subparsers.add_parser(
        "accept_policy",
        help="Accept the session policy",
    )
    _add_connection_args(accept_parser)
    accept_parser.add_argument(
        "--token",
        default=os.environ.get("ZTAB_TOKEN"),
        help=(
    ...
    status_parser = subparsers.add_parser(
        "get_status",
        help="Get current session status",
    )
...
def main():
    ...
    if hasattr(args, "token") and not args.token:
        print(
            "Error: The --token flag or ZTAB_TOKEN "
    ...
    try:
        commands = {
            ...
            "join_session": cmd_join_session,
            "accept_policy": cmd_accept_policy,
            "submit_input": cmd_submit_input,
