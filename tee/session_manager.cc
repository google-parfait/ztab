...
// Finding 2: Security limit for nested JSON parsing to prevent stack overflow DoS.
...
std::string StripMarkdownCodeFences(const std::string& input) {
  std::string s = input;
  std::string content;
  // Match the first ``` block, optionally with a language specifier on the first line.
...
absl::StatusOr<nlohmann::json> ParseJsonSafely(const std::string& json_str) {
  try {
    return nlohmann::json::parse(
        json_str, [](int depth, nlohmann::json::parse_event_t, nlohmann::json&) {
          return depth <= kMaxJsonParseDepth;
        });
    ...
    if (schema.contains("additionalProperties") &&
      ...
      for (auto& [key, _] : value.items()) {
        if (!schema.contains("properties") || !schema["properties"].contains(key)) {
          return absl::InvalidArgumentError("Unexpected field present in object");
        }
      }
    }
    // Validate properties.
...
std::string SessionManager::GenerateRandomHex(int num_bytes) {
  ...
  if (RAND_bytes(buf.data(), num_bytes) != 1) {
    LOG(FATAL) << "RAND_bytes failed! Cryptographically secure RNG unavailable.";
  }
...
bool SessionManager::CheckAndEnforceTimeout(Session& session) {
  ...
  if (session.state == OPEN) {
    ...
    // resource exhaustion DoS (where attackers create empty sessions to fill slots).
    effective_timeout = std::min(effective_timeout, absl::Seconds(kMaxOpenSeconds));
  } else if (session.state == SEALED) {
    // Finding 3: Limit the time a session can sit in the SEALED state waiting
    // for inputs, preventing long-timeout sessions from locking up slots.
    effective_timeout = std::min(effective_timeout, absl::Seconds(kMaxSealedSeconds));
  ...
  if (elapsed > effective_timeout) {
    ...
    session.error_detail = absl::StrCat(
        "Session timed out in state ", old_state_name,
        " after ", absl::FormatDuration(elapsed));
    session.state_entered_at = absl::Now();
    LOG(WARNING) << "Session " << session.id << " timed out: "
...
absl::StatusOr<std::pair<SessionManager::Session*, SessionManager::Participant*>>
SessionManager::Authenticate(const std::string& session_id,
                             const std::string& participant_token) {
  auto session_it = sessions_.find(session_id);
  if (session_it == sessions_.end()) {
    // Uniform PERMISSION_DENIED — don't leak session existence.
    return absl::PermissionDeniedError("Invalid session or participant token");
  }
...
void SessionManager::SweepTerminalSessions() {
  ...
  while (it != sessions_.end()) {
    ...
    if (s.state == CLOSED || s.state == ABORTED) {
      if (s.state == ABORTED && (s.error_code == JOIN_TIMEOUT || s.error_code == INPUT_TIMEOUT)) {
        // Unauthenticated or partially-authenticated sessions that stalled. Delete immediately
        // instead of waiting 1 hour to prevent resource exhaustion DoS.
        should_erase = true;
      } else if ((now - s.state_entered_at) > absl::Seconds(kTerminalRetentionSeconds)) {
        should_erase = true;
      }
    }
...
absl::StatusOr<CreateSessionResponse> SessionManager::CreateSession(
  ...
  if (static_cast<int>(sessions_.size()) >= kMaxSessions) {
    return absl::ResourceExhaustedError(
        absl::StrCat("Server at session capacity (", kMaxSessions,
                     "). Try again later."));
  }
  ...
  if (policy.expected_participants() > kMaxParticipants) {
    return absl::InvalidArgumentError(
        absl::StrCat("expected_participants exceeds maximum (",
                     kMaxParticipants, ")"));
  }
  ...
  if (policy.timeout_seconds() > kMaxTimeoutSeconds) {
    return absl::InvalidArgumentError(
        absl::StrCat("timeout_seconds exceeds maximum (",
                     kMaxTimeoutSeconds, ")"));
  }
  ...
  if (!input_schema.empty()) {
    ...
    if (!parsed_or.ok()) {
      return absl::InternalError(
          absl::StrCat("Malformed built-in input_schema_json: ", parsed_or.status().message()));
    }
  }
  const std::string& output_schema = (*policy_def_or)->default_output_schema_json;
  if (!output_schema.empty()) {
    auto parsed_or = ParseJsonSafely(output_schema);
    if (!parsed_or.ok()) {
      return absl::InternalError(
          absl::StrCat("Malformed built-in output_schema_json: ", parsed_or.status().message()));
    }
  }

  // TODO(Phase 2): Implement per-client rate limiting and/or mTLS authentication
  // to prevent unauthenticated session exhaustion DoS. Currently outside scope.
  ...
  session.state_entered_at = absl::Now();
  int timeout_secs = policy.timeout_seconds() > 0
                         ? policy.timeout_seconds()
                         : kDefaultTimeoutSeconds;
  session.timeout = absl::Seconds(timeout_secs);
  session.policy.set_timeout_seconds(timeout_secs);

  // Register creator as first participant (already accepted — they set the policy).
  Participant creator;
  ...
  LOG(INFO) << "Session " << session_id << " created (policy="
            << policy.policy_class()
...
absl::StatusOr<JoinSessionResponse> SessionManager::JoinSession(
  ...
  if (session_it == sessions_.end()) {
    return absl::PermissionDeniedError("Invalid session or session not available for joining");
  }
  Session& session = session_it->second;
...
absl::StatusOr<AcceptPolicyResponse> SessionManager::AcceptPolicy(
  ...
  auto auth_or = Authenticate(request.session_id(),
                              request.participant_token());
  ...
  if (session->state != OPEN) {
    return absl::FailedPreconditionError(
        absl::StrCat("Session is not OPEN (state=",
...
absl::StatusOr<SubmitInputResponse> SessionManager::SubmitInput(
...

  {
    ...
    if (session->state != SEALED) {
      ...
      return absl::FailedPreconditionError(
          absl::StrCat("Session is not SEALED (state=",
                       SessionState_Name(session->state), ")"));
    }
    ...
    if (participant->input_submitted) {
      ...
      SubmitInputResponse response;
      response.set_state(session->state);
      response.set_remaining_inputs(
          session->policy.expected_participants() - submitted);
      return response;
    }
    ...
    if (static_cast<int>(request.input_json().size()) > kMaxInputBytes) {
      return absl::InvalidArgumentError(
          absl::StrCat("Input payload too large (",
                       request.input_json().size(), " bytes, max ",
                       kMaxInputBytes, ")"));
    }

    // Normalize input by parsing and dumping to strip duplicate keys, comments, etc.
    // This prevents parser differentials (e.g., duplicate keys used for prompt injection).
    ...
    if (!input_schema.empty()) {
      auto validate_status = ValidateJsonSchema(parsed_input.dump(),
                                                input_schema);
      if (!validate_status.ok()) {
        return absl::InvalidArgumentError(
            absl::StrCat("Input schema violation: ",
    ...
    LOG(INFO) << "Session " << session->id << ": input submitted ("
              << submitted << "/" << session->policy.expected_participants()
              << ")";
    ...
    if (remaining == 0) {
      ...
      should_process = true;
      ++active_async_threads_; // Track thread inside main lock to prevent use-after-free
    } else {
  ...
  if (should_process) {
    try {
      ...
      if (it != sessions_.end()) {
        ...
        it->second.error_detail = absl::StrCat("Failed to spawn inference thread: ", e.what());
      }
      return absl::InternalError("Failed to spawn LLM background thread: resource exhausted");
    }
  }
...
void SessionManager::ProcessSessionAsync(const std::string& session_id) {
  ...
  std::string output_schema;
  {
    ...
    std::sort(sorted_tokens.begin(), sorted_tokens.end(), [&session](const std::string& a, const std::string& b) {
      return session.participants.at(a).join_index < session.participants.at(b).join_index;
    });
    ...
    for (const auto& token : sorted_tokens) {
      ...
      inputs_block << "<<<PARTICIPANT_" << participant_num
                   << "_INPUT_BEGIN_" << delim_suffix << ">>>\n"
                   << p.input_json << "\n"
                   << "<<<PARTICIPANT_" << participant_num
                   << "_INPUT_END_" << delim_suffix << ">>>\n\n";
      ++participant_num;
    }
    prompt = session.policy_def->prompt_template;
    prompt = absl::StrReplaceAll(prompt, {
        {"{num_participants}",
         std::to_string(session.participants.size())},
    ...
    LOG(INFO) << "Session " << session.id
              << ": aggregated prompt (" << prompt.size() << " bytes)";
  }
